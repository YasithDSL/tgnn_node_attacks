...
ATTACKS = ['evasion_random', 'evasion_heterophilic', 'evasion_hub_cascade']
SEEDS   = [1, 2, 3]

BUDGET_SWEEP_BUDGETS = [1_000, 2_900, 5_800]
BAR_BUDGET           = 2_900
...
def run_one(model, attack, budget, seed, device, epochs, script_dir):
    ...
    print(f"\n{'─'*64}")
    print(f"  {model:5s} | {attack:25s} | budget={budget:>6,} | seed={seed} | epochs={epochs}")
    print(f"{'─'*64}")
    t0 = time.time()
    try:
...
if __name__ == '__main__':
    ...
    p.add_argument('--results-file', default='results_evasion.json')
    ...
    for model, attack, budget in sorted(all_experiments()):
        ...
        if vals:
            ...
            mean_s = std_s = '—'
        print(f"{model:<6} {attack:<27} {budget:>8,}  {mean_s:>10}  {std_s:>7}  {len(vals):>10}")